...
test "PackedIntArray at end of available memory" {
    ...
    const Padded = struct {
        _: [std.heap.min_page_size - @sizeOf(PackedArray)]u8,
        p: PackedArray,
    };
...
test "PackedIntSlice at end of available memory" {
    ...
    var page = try allocator.alloc(u8, std.heap.min_page_size);
    defer allocator.free(page);

    var p = PackedSlice.init(page[std.heap.min_page_size - 2 ..], 1);
