...
pub const min_page_size = switch (builtin.cpu.arch) {
    ...
    // Todo: might be causing the test runner to infinitely fail?
...
pub fn alignPageAllocLen(full_len: usize, len: usize) usize {
    ...
    assert(mem.alignForward(usize, aligned_len, min_page_size) == full_len);
    return aligned_len;
}
...
test "PageAllocator" {
    ...
    {
        var buf = try allocator.alloc(u8, min_page_size + 1);
...
pub fn testAllocatorAlignedShrink(base_allocator: mem.Allocator) !void {
    ...
    const alloc_size = min_page_size * 2 + 50;
    ...
    while (@intFromPtr(slice.ptr) == mem.alignForward(usize, @intFromPtr(slice.ptr), min_page_size * 32)) {
