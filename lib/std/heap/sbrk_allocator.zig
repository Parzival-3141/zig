...
pub fn SbrkAllocator(comptime sbrk: *const fn (n: usize) usize) type {
    return struct {
        ...
        const pages_per_bigpage = bigpage_size / std.heap.min_page_size;
        ...
        fn alloc(ctx: *anyopaque, len: usize, log2_align: u8, return_address: usize) ?[*]u8 {
            ...
            if (class < size_class_count) {
                const addr = a: {
                    ...
                    if (next_addr % std.heap.min_page_size == 0) {
        ...
        fn allocBigPages(n: usize) usize {
            ...
            return sbrk(pow2_pages * pages_per_bigpage * std.heap.min_page_size);
        }
    };
}
