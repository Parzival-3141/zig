...
fn nPages(memsize: usize) usize {
    return mem.alignForward(usize, memsize, min_page_size) / min_page_size;
}

fn alloc(ctx: *anyopaque, len: usize, log2_align: u8, ra: usize) ?[*]u8 {
    _ = ctx;
    _ = ra;
    if (len > maxInt(usize) - (min_page_size - 1)) return null;
    const page_count = nPages(len);
    const page_idx = allocPages(page_count, log2_align) catch return null;
    return @as([*]u8, @ptrFromInt(page_idx * min_page_size));
}
...
fn allocPages(page_count: usize, log2_align: u8) !usize {
    ...
    const next_page_addr = next_page_idx * min_page_size;
    const aligned_addr = mem.alignForwardLog2(next_page_addr, log2_align);
    const drop_page_count = @divExact(aligned_addr - next_page_addr, min_page_size);
...
fn freePages(start: usize, end: usize) void {
    ...
    if (end > extendedOffset()) {
        ...
        if (!extended.isInitialized()) {
            ...
            extended.data = @as([*]u128, @ptrFromInt(new_end * min_page_size))[0 .. min_page_size / @sizeOf(u128)];
...
fn resize(
    ...
    _ = log2_buf_align;
    _ = return_address;
    const aligned_len = mem.alignForward(usize, buf.len, min_page_size);
...
test "internals" {
    ...
    const conventional_memsize = WasmPageAllocator.conventional.totalPages() * min_page_size;
    const initial = try page_allocator.alloc(u8, min_page_size);
