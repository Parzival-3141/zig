...
pub const ElfDynLib = struct {
    ...
    versym: ?[*]u16,
    verdef: ?*elf.Verdef,
    memory: []align(std.heap.min_page_size) u8,
    ...
    pub fn open(path: []const u8) !ElfDynLib {
        ...
        const file_bytes = try os.mmap(
            null,
            mem.alignForward(usize, size, std.heap.min_page_size),
            os.PROT.READ,
            os.MAP.PRIVATE,
            fd,
            0,
            ...
            while (i < eh.e_phnum) : ({
                ...
                switch (ph.p_type) {
                    elf.PT_LOAD => {
                        ...
                        const aligned_addr = (base + ph.p_vaddr) & ~(@as(usize, std.heap.min_page_size) - 1);
                        const extra_bytes = (base + ph.p_vaddr) - aligned_addr;
                        const extended_memsz = mem.alignForward(usize, ph.p_memsz + extra_bytes, std.heap.min_page_size);
                        const ptr = @as([*]align(std.heap.min_page_size) u8, @ptrFromInt(aligned_addr));
